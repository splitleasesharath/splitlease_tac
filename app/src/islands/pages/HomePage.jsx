...
function ScheduleSection() {
  const schedules = [
    ...
    },
    {
      ...
      days: '5,6,0,1', // Fri-Sun+Mon (0-based)
    },
    {
      type: 'monthly',
...
function ListingsPreview({ selectedDays }) {
  ...
  const handleListingClick = (propertyId) => {
    // Use 0-based indexing for URL parameter (consistent with JavaScript Date API)
  ...
  const handleShowMore = () => {
    // Use 0-based indexing for URL parameter (consistent with JavaScript Date API)
    const daysParam = selectedDays.length > 0 ? selectedDays.join(',') : '0,1,2,3,4,5,6';
...
export default function HomePage() {
  ...
  const loadStateFromURL = () => {
    ...
    if (daysParam) {
      ...
      // Parse 0-based day indices directly (consistent with JavaScript Date API)
      const dayIndices = decoded.split(',').map((d) => parseInt(d.trim())).filter(d => !isNaN(d) && d >= 0 && d <= 6);
